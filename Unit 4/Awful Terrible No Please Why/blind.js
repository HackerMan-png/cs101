...
function claimSpace(space) {
	...
	if (document.getElementById(`button${space}`).classList.contains("usedX") || document.getElementById(`button${space}`).classList.contains("usedO")) {
		...
		document.getElementById(`button${space}`).textContent = "?";
	...
	if (player == "X") {
		player = "O";
		document.getElementById("player").textContent = "Player: Player 2";
	} else {
		player = "X";
...
function winner(winner) {
	...
	if (winner == "X") {
		xWins++;
		document.getElementById("xWins").textContent = `Player 1 - ${xWins} wins`;
	} else if (winner == "O") {
		oWins++;
		document.getElementById("oWins").textContent = `Player 2 - ${oWins} wins`;
	}
	playing = false
}
